...
api_location_file_path = os.path.join(base_path, 'API_LOCATION_V2.csv')

# 수정된 경로를 사용하여 CSV 파일 불러오기
try:
    API_LOCATION = pd.read_csv(api_location_file_path)
...
while start_date <= end_date:
    ...
    # 사용자의 홈 디렉터리에 파일 저장 경로 설정
    user_desktop = os.path.join(os.path.expanduser('~'), 'Desktop')
    weather_save_file_name = f"Weather_{start_date_str}_file_pre.csv"
    weather_save_file_path = os.path.join(user_desktop, 'Battery_Charging_Algorithm', 'API', weather_save_file_name)

    # 저장할 폴더가 없는 경우 생성
    weather_save_directory = os.path.dirname(weather_save_file_path)
    if not os.path.exists(weather_save_directory):
        os.makedirs(weather_save_directory)  # 폴더 생성
...
# PyInstaller 실행 환경을 고려한 경로 설정
base_path = getattr(sys, '_MEIPASS', os.path.dirname(os.path.abspath(__file__)))
api_location_file_path = os.path.join(base_path, 'API_FORECAST_REGION_V2.csv')

# 수정된 경로를 사용하여 CSV 파일 불러오기
try:
    API_LOCATION_FORECAST = pd.read_csv(api_location_file_path)
except Exception as e:
    print(f"{e}")
...
# 사용자의 홈 디렉터리에 파일 저장 경로 설정
user_desktop = os.path.join(os.path.expanduser('~'), 'Desktop')
forecast_save_file_name = f"forecast_{end_date_str}_file_pre.csv"
forecast_save_file_path = os.path.join(user_desktop, 'Battery_Charging_Algorithm', 'API', forecast_save_file_name)
# 저장할 폴더가 없는 경우 생성
forecast_save_directory = os.path.dirname(forecast_save_file_path)
if not os.path.exists(forecast_save_directory):
    os.makedirs(forecast_save_directory)  # 폴더 생성
